...
The next task is to explore the data by profiling the fields of interest - in this case to understand the makeup of the data related to ambience. As can be seen in the summary below, it can be seen that compared to the `r prettyNum(nrow(businessdata), big.mark=",")` rows in the dataset the ambience data is very sparsely populated.  This makes sense as it would not apply to many categories of business.
...
```
For the Random Forest model the accuracy was unchanged at `r rf_results2$overall['Accuracy']` and unchanged for the Naive Bayes model at `r nb_results2$overall['Accuracy']`
